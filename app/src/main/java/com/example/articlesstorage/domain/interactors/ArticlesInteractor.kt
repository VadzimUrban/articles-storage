...
class ArticlesInteractor(private val articlesRepository: ArticlesRepository) {
    ...
    fun getArticlesByTitleFlow(title: String): Flow<List<Article>> =
        articlesRepository.getArticlesByTitleFlow(title)

    suspend fun setArticle(article: Article) = articlesRepository.setArticle(article)
    ...
    fun sortArticlesByYearFlow(year: Year): Flow<List<Article>> =
        articlesRepository.sortArticlesByYearFlow(year)

    fun sortArticlesByAuthorLastnameFlow(lastname: String): Flow<List<Article>> =
        articlesRepository.sortArticlesByAuthorLastnameFlow(lastname)
}